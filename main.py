...
def updater_path():
    """Путь к Updater.exe рядом с exe (сборка) или рядом с main.py (разработка)."""
        ...
        if latest_version != APP_VERSION:
            ...
            if msg.clickedButton() == download_btn:
                ...
                if os.path.isfile(up):
                    subprocess.Popen([up], cwd=os.path.dirname(up))
                ...
                if remind_dt <= now_dt:
                    ...
                    if WIN_TOAST_AVAILABLE:
                        ...
                        notifier.show_toast(
                            ...
                            text or "",
                            icon_path=resource_path("images", "icons", "Frame 1.png"),
                            duration=10,
                            threaded=True,
...
class NoterraApp(QWidget):
    def __init__(self):
        ...
        app.setWindowIcon(QIcon(resource_path("images", "icons", "Frame 1.png")))
        ...
        self.filter = self.ui.findChild(QComboBox,"FilterBox")
        self.filter_add = self.ui.findChild(QLineEdit,"Filter_add")
        self.category_model = QStringListModel(self)
        self.category_completer = QCompleter(self.category_model, self)
        ...
        self.catalog_label = self.ui.findChild(QLabel,"Catg")
        self.catalog_label.setAttribute(Qt.WA_TransparentForMouseEvents)

        self.scr2.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)

        # Не удаляем корзину автоматически: это может приводить к потере данных.
        # Очистка корзины остаётся только ручной по кнопке "delete_all".
    ...
    def load_today_events(self):
        ...
        # --- Фильтруем и корректно сортируем только сегодняшние заметки ---
        today_str = datetime.now().strftime("%d-%m-%Y")
        today_reminders = []
        for r in reminders:
            note_id, title, text, remind_at = r
            try:
                remind_dt = datetime.strptime(remind_at, "%d-%m-%Y %H:%M")
            except (TypeError, ValueError):
                continue

            if remind_dt.strftime("%d-%m-%Y") == today_str:
                today_reminders.append((r, remind_dt))

        today_reminders.sort(key=lambda item: item[1])
        today_reminders = [item[0] for item in today_reminders]
    ...
    # Получаем заголовок и убираем пробелы
            self.HOMEE()
            textfilter = self.filter_add.text().strip()
            if not textfilter:
                textfilter = "Без категории"
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO reminders (title, text, remind_at, remind_at2,category) VALUES (?, ?, ?, ?, ?)",
                (title, text, remind_at, remind_at2,textfilter)
            )
            conn.commit()
            conn.close()
    ...
    def load_notes(self):
        selected = (self.filter.currentText() or "All").strip()
        if not selected:
            selected = "All"
        ...
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute("SELECT id, title, text, remind_at, remind_at2, isDone, category FROM reminders")
        reminders = c.fetchall()
        conn.close()

        row = 0

        for idx, (note_id, title, text, remind_at, remind_at2, isDone,category) in enumerate(reminders):
            current_category = (category or "").strip() or "Без категории"

            if selected != "All" and current_category != selected:
        ...
        self.load_combobox(selected_category=selected)
    ...
    def mark_done(self, note_id, checked):
        ...
        if self.language == "ru":
            msg.setWindowTitle("Подтверждение выполнения")
            msg.setText("Вы уверены, что хотите подтвердить удаление этой заметки?")
    ...
    # Создаем сообщение
        ...
        cursor.execute(
            "SELECT title, text, remind_at, remind_at2,category FROM reminders WHERE id = ?",
            (reminder_id,)
        )
        ...
        if note:
            ...
            cursor.execute("""
                INSERT INTO trash 
                (title, text, remind_at, remind_at2, deleted_at, isDeleted, isDone, category)
                VALUES (?, ?, ?, ?, ?, 1, 0, ?)
            """, (title, text, remind_at, remind_at2, deleted_at,category))
    ...
    def load_combobox(self, selected_category="All"):
        ...
        idx = self.filter.findText(selected_category)
        if idx < 0:
            idx = 0
        self.filter.setCurrentIndex(idx)
        self.filter.blockSignals(False)
        self.category_model.setStringList(category_values)
    ...
    def load_trash(self):
        ...
        # ВАЖНО: берём isDone
        c.execute("""
            SELECT id, title, text, remind_at, remind_at2, isDone,category
            FROM trash
        """)
    ...
    def hide_label5(self):
        if self.filter_add.text().strip():
    ...
    def check_expired_notes(self):
        ...
        cursor.execute("SELECT id, title, text, remind_at FROM reminders")
        all_notes = cursor.fetchall()
        expired_notes = []
        for note_id, title, text, remind_at in all_notes:
            try:
                remind_dt = datetime.strptime(remind_at, "%d-%m-%Y %H:%M")
            except (TypeError, ValueError):
                continue
            if remind_dt <= now_dt:
                expired_notes.append((note_id, title, text, remind_at))
    ...
    def restore_from_trash(self, trash_id):
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute("SELECT title, text, remind_at, remind_at2,category FROM trash WHERE id = ?", (trash_id,))
        note = c.fetchone()
