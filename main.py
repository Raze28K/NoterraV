...
APP_VERSION = "0.9.0"  # твоя текущая версия

def check_update():
    try:
        url = "https://api.github.com/repos/Raze28K/Nottera/releases/latest"
        response = requests.get(url, timeout=10)

        if response.status_code != 200:
            return  # если релиза нет или ошибка сети
        ...
        if latest_version != APP_VERSION:
            ...
            if msg.clickedButton() == download_btn:
                webbrowser.open(data["html_url"])  # открыть страницу релиза

        else:
            # если хочешь показывать сообщение:
            # QMessageBox.information(None, "Обновление", "У вас последняя версия")
            pass

    except requests.exceptions.RequestException:
        print("Нет интернета или GitHub недоступен")
...
class NoterraApp(QWidget):
    ...
    # Настройки отступов и расстояний
        ...
        self.calendar.setStyleSheet("""
            ...
            /* Нормальные дни текущего месяца */
            QCalendarWidget QAbstractItemView:enabled {
                color: white;
            }

            /* Дни из другого месяца — серые */
    ...
    def save_note_changes(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE reminders SET title=?, text=?, remind_at=? WHERE id=?",
